...
**What it is:**
Rendering volumetric light scattering (god rays, fog) in screen space at reduced resolution (half or quarter), then upsampling to full resolution using bilateral filtering that respects depth edges.

**Why it matters for Toaster:**
If Toaster's runtime fog shader becomes a performance bottleneck (especially on mobile/VR), rendering the ray march at quarter resolution with bilateral upsampling can reduce cost by 8-16x while preserving edge quality.

**Key implementation details:**
- **Downscale**: render the fog ray march into a quarter-res (1/4 width, 1/4 height) render target.
- **Bilateral upsample**: weight samples by `exp(-|depth_full - depth_quarter| / sigma)`. Preserves hard edges at geometry boundaries.
- **Temporal upsampling alternative**: render at quarter-res with a different sub-pixel offset each frame, accumulate over 4 frames.
- **Performance**: Wronski reports ~1.1ms total for full volumetric fog at quarter-res on PS4/Xbox One.
...
**Why it matters for Toaster:**
Cone tracing could let Toaster provide real-time indirect lighting. If the `LightingGrid` is extended with a mip chain, the fog shader or a surface shader can trace cones to gather soft bounced light. Deterministic — no random rays, no noise, no temporal accumulation needed.

**Key implementation details:**
- **Mip chain**: generate mipmaps for the 3D texture. Each mip level averages 2x2x2 voxels. Use anisotropic mipmapping for better directional filtering.
- **Cone marching**: diffuse = 5-9 cones distributed over hemisphere (60-90 degree aperture each). Specular = 1 cone in reflection direction.
- **Mip selection**: `mipLevel = log2(2 * t * tan(theta/2) / voxelSize)`.
- **Cost**: ~2-4ms for diffuse + specular on a 256^3 grid at 1080p.
- **Limitations**: light leaking through thin geometry, overly soft at low resolution.
...
Replace voxels with surfels (surface elements) placed on geometry. More memory-efficient for surface-only lighting. Reference: UE5 Lumen documentation.

### Probe-Based Irradiance (DDGI)
Dynamic Diffuse Global Illumination — grid of spherical probes that store irradiance as octahedral maps. Real-time update via ray tracing. Reference: Majercik et al. (2019).

### Photon Mapping
Bidirectional approach — trace photons from lights, deposit in voxel grid, then gather during camera ray march. Handles caustics naturally. Reference: Jensen, "Realistic Image Synthesis Using Photon Mapping" (2001).
